...
\author{David E. England, PhD and 5DGSPTSBL Contributors}
\date{2026-08-04}
...
\begin{landscape}
\includegraphics[width=\linewidth]{figures/fig0_tikz.pdf}
\end{landscape}
...
This implies the invariant fast-slow geometric structure is preserved under the chart.
\end{proof}
...
\paragraph{Synoptic Shear Forcing $\mathcal{F}_S$.}
The slow momentum forcing function $\mathcal{F}_S$ in \eqref{eq:slow_S} represents the synoptic and mesoscale driving of vertical wind shear $S = \partial U / \partial z$ against turbulent momentum depletion. In the GSPT framework, $\mathcal{F}_S$ is closed via a relaxational geostrophic driving law:
\begin{equation}
\mathcal{F}_S(S) = \gamma_S \left( S_{\text{geo}} - S \right),
\label{eq:shear_forcing_closure}
\end{equation}
where $S_{\text{geo}} = \frac{g}{f_0 T_0} \left| \nabla_h T \right|$ is the background geostrophic shear maintained by horizontal thermal advection or low-level jet (LLJ) inertial oscillations, and $\gamma_S \sim \mathcal{O}(10^{-4}\,\mathrm{s}^{-1})$ is the slow momentum relaxation rate.
...
The interior dissipation/production scale is set by \eqref{eq:blackadar_length} while neutral bulk drag/heat exchange coefficients $C_D$ and $C_H$ act only as lower-boundary transfer closures at $z_1$. This separates boundary drag physics from interior turbulent mixing geometry.
